...
def featurize_lfp(lfp_data, bin_size=200, samp_freq=2500, batch_size=50, pc_var_thresh=0.99):
    ...
    for name in ['psd_lfp']:
        ...
        #K, T, C = binned_bands.shape
        #scaler = StandardScaler()
        #bands_scaled = scaler.fit_transform(binned_bands.reshape((-1, C)))
        #pca = PCA(n_components=C)  
        #bands_transformed = pca.fit_transform(bands_scaled)
        #n_pcs = np.argwhere(np.cumsum(pca.explained_variance_ratio_) <= pc_var_thresh).flatten().max()
        #bands_transformed = bands_transformed[:,:n_pcs].reshape(K, T, n_pcs)

        #all_psd[name] = bands_transformed
        #del bands_transformed
